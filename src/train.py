...
class Trainer:
    ...
    def get_loss(self, pafs, kps, o1, o2):
        loss_paf = sum([self.deep_supervion_decay**i * self.paf_loss_multiplier * self.paf_loss(o.float(), pafs.float())
                        for i, o in enumerate(o1[::-1])])
        loss_kp = sum([self.deep_supervion_decay**i * self.kp_loss_multiplier * self.kp_loss(o.float(), kps.float())
    ...
    def train(self):
        ...
        for epoch in range(config['default'].getint('epochs')):
            ...
            with torch.no_grad():
                for data in tqdm(self.val_dataloader, total=self.val_steps, desc=f'[VAL] Epoch {epoch}'):
                    imgs, pafs, kps = data
                    imgs, pafs, kps = imgs.cuda(), pafs.cuda(), kps.cuda()
